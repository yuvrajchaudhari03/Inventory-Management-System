...
def addproductmovement():
    if (request.method == 'POST'):
        ...
        if(from_location==None):
            data = Productmovement(from_location=from_location, to_location=to_location, qty=quantity,
                                   timestamp=timestamp, product_id=product_id, location_id=location_id)
            db.session.add(data)
            db.session.commit()
        elif (results2[3] < int(quantity)):
            flash("Low Quantity!!")
        else:
